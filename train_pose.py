...
import argparse
import time
from utils import adjust_learning_rate, AverageMeter, save_checkpoint, dataset, transforms
...
def parse():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        help='optional config.OPOSE file',
                        default='./cfgs/opose/vgg19s8_cpm6s.yml', type=str)
    parser.add_argument('--set', dest='set_cfgs',
                        help='set config.OPOSE keys', default=None,
...
def get_parameters(model, config, isdefault=True):
    ...
    for key, value in params_dict.items():
        if ('model1_' not in key) and ('model0.' not in key):
            ...
            lr_1.append(value)
    params = [{'params': lr_1, 'lr': config.OPOSE.base_lr},
            {'params': lr_2, 'lr': config.OPOSE.base_lr * 2.},
            {'params': lr_4, 'lr': config.OPOSE.base_lr * 4.},
            {'params': lr_8, 'lr': config.OPOSE.base_lr * 8.}]
...
def train_val(model, args, cfg):
    ...
    train_loader = torch.utils.data.DataLoader(
            dataset.CocoFolder(traindir, 8,
                transforms.Compose([transforms.RandomResized(),
                transforms.RandomRotate(40),
                transforms.RandomCrop(368),
                transforms.RandomHorizontalFlip(),
            ])),
            batch_size=config.OPOSE.batch_size, shuffle=True,
            num_workers=config.OPOSE.workers, pin_memory=True)

    if config.OPOSE.test_interval != 0 and args.val_dir is not None:
        val_loader = torch.utils.data.DataLoader(
                dataset.CocoFolder(valdir, 8,
                    transforms.Compose([transforms.TestResized(368),
                ])),
                batch_size=config.OPOSE.batch_size, shuffle=False,
                num_workers=config.OPOSE.workers, pin_memory=True)
    ...
    optimizer = torch.optim.SGD(params, config.OPOSE.base_lr, momentum=config.OPOSE.momentum,
                                weight_decay=config.OPOSE.weight_decay)
    ...
    iters = config.OPOSE.start_iters
    best_model = config.OPOSE.best_model
    learning_rate = config.OPOSE.base_lr
    ...
    while iters < config.OPOSE.max_iter:
    
        for i, (input, heatmap, vecmap, mask) in enumerate(train_loader):

            learning_rate = adjust_learning_rate(optimizer, iters, config.OPOSE.base_lr, policy=config.OPOSE.lr_policy, gamma = config.OPOSE.gamma, step_size = config.OPOSE.step_size, multiple=multiple)
            ...
            iters += 1
            if iters % config.OPOSE.display == 0:
                print('Train Iteration: {0}\t'
                    'Time {batch_time.sum:.3f}s / {1}iters, ({batch_time.avg:.3f})\t'
                    'Data load {data_time.sum:.3f}s / {1}iters, ({data_time.avg:3f})\n'
                    'Learning rate = {2}\n'
                    'Loss = {loss.val:.8f} (ave = {loss.avg:.8f})\n'.format(
                    iters, config.OPOSE.display, learning_rate, batch_time=batch_time,
            ...
            if config.OPOSE.test_interval != 0 and args.val_dir is not None and iters % config.OPOSE.test_interval == 0:
            ...
            if iters == config.OPOSE.max_iter:
                break
