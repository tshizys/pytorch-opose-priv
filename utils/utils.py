...
def adjust_learning_rate(optimizer, iters, base_lr, gamma,step_size, policy='step', multiple=[1]):
    ...
    if policy == 'fixed':
        ...
        lr = base_lr * (gamma ** iters)
    # elif policy == 'inv':
    #     lr = base_lr * ((1 + gamma * iters) ** (-policy_parameter['power']))
    ...
    # elif policy == 'sigmoid':
    #     lr = base_lr * (1.0 / (1 + math.exp(-gamma * (iters - policy_parameter['stepsize']))))
    ...
    #             stepend = stepvalue
    #             break
    #     lr = max(lr * gamma, lr * (1 - (iters - stepstart) * 1.0 / (stepend - stepstart)) ** policy_parameter['power'])
